package com.example.animcare.MainActivityFiles.MyPets.Fragments
...
import com.example.animcare.MainActivityFiles.MyPets.Adapters.MyPetsAdapter
import com.example.animcare.MainActivityFiles.MyPets.Pet
import com.example.animcare.MainActivityFiles.MyPets.PetInList
...
import kotlinx.android.synthetic.main.quiz_type_position_recycler_view.*
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()){
                   ...
                   for (pet in snapshot.children){
                    ...
                    recycler_view_pets_fregment.adapter = MyPetsAdapter(requireContext().applicationContext, userPets, false)
...
//                                recycler_view_pets_fregment.layoutManager = LinearLayoutManager(requireContext().applicationContext)
                                if (it.isChecked){
                                    ...
                                    recycler_view_pets_fregment.adapter = MyPetsAdapter(requireContext().applicationContext, userPets, true)
                                    ...
                                    println("Wciśnięte")
                                    removing_info_TW.text = "${requireActivity().applicationContext.getString(R.string.removing_elements_info)}"
                            ...
                            R.id.returnRemovePet -> {
                                ...
                                removing_info_TW.text = "${requireActivity().applicationContext.getString(R.string.myPets)}"
    ...
    private val setOnItemSelectedListener: NavigationBarView.OnItemSelectedListener =
        NavigationBarView.OnItemSelectedListener {
            ...
            when(it.itemId) {
                ...
                R.id.returnRemovePet -> {
                    recycler_view_pets_fregment.adapter = MyPetsAdapter(requireContext().applicationContext, arrayListOf<PetInList>(), false)
                    it.isVisible = false
                    requireActivity().bottomNavMyPet.menu.getItem(0).isVisible = true
                    requireActivity().bottomNavMyPet.menu.getItem(1).isVisible = true
                    requireActivity().actionBar?.displayOptions = ActionBar.DISPLAY_SHOW_TITLE
...
//        recycler_view_pets_fregment.adapter = MyPetsAdapter(requireContext().applicationContext, PetInList, false)
    ...
    private fun removeLastFragmentInStack(fragment: Fragment){
        if (parentFragmentManager.fragments.isNotEmpty()){
            val fragmentName= parentFragmentManager.fragments[parentFragmentManager.fragments.size-1].javaClass.name
